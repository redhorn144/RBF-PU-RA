...
def NormalizeWeightsEval(comm, patches, patches_for_rank, eval_pts):
    """
    Compute and store normalized PU weights at the collocation (eval) points.

    Populates patch.w_bar_eval, patch.gw_bar_eval, patch.lw_bar_eval for each
    patch, using the same Wendland C2 functions as NormalizeWeights but
    evaluated at eval_pts (shape M×d) rather than solution nodes.
    """
    M = eval_pts.shape[0]
    d = eval_pts.shape[1]

    W_local     = np.zeros(M)
    gradW_local = np.zeros((M, d))
    lapW_local  = np.zeros(M)

    for patch in patches:
        idx        = patch.eval_indices
        pts        = eval_pts[idx]

        w  = C2Weight(pts, patch.center, patch.radius)
        gw = C2WeightGradient(pts, patch.center, patch.radius)
        lw = C2WeightLaplacian(pts, patch.center, patch.radius)
    ...
    for patch in patches:
        ...
        patch.w_bar_eval  = w / Wn
        patch.gw_bar_eval = gw / Wn[:, None] - w[:, None] * gWn / Wn[:, None]**2
        patch.lw_bar_eval = (lw / Wn
                             - 2.0 * np.sum(gw * gWn, axis=1) / Wn**2
                             - w * lWn / Wn**2
                             + 2.0 * w * np.sum(gWn * gWn, axis=1) / Wn**3)
